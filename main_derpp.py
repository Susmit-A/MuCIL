...
prev_concept_classifier = None
prev_dataset = None
prev_num_concepts = None
prev_num_classes = 0
...
def train_joint(args, mm_model, image_model, concept_projector, concept_classifier, cls_embs, concept_label_dict, concept_emb_dict, exemplars):
    global prev_task_model, prev_concept_classifier, prev_dataset, prev_num_concepts, prev_num_classes
        ...
        for step, data in enumerate(trainloader):
            ...
            with torch.no_grad():
                if args.experience > 0:
                    ...
                    imgs = torch.cat([imgs, er_imgs], dim=0)
                    cls = torch.cat([cls, er_cls], dim=0)
                    attrs = torch.cat([attrs, er_attrs], dim=0)
                    attr_embs = torch.cat([attr_embs, er_attr_embs], dim=0)
                
                    prev_task_feats = image_model(pixel_values=er_imgs.to('cuda:0')).last_hidden_state
                    prev_task_feats = torch.cat([projector(prev_task_feats), er_attr_embs.cuda()], dim=1)
                    prev_output_feats = prev_task_model(hidden_states=prev_task_feats).last_hidden_state
                    prev_concepts = prev_output_feats[:, -prev_dataset.get_concept_count():, :].cuda()[:, :prev_num_concepts, :]
                    prev_concept_values = prev_concept_classifier(prev_concepts)
                    prev_classes = torch.bmm(prev_concepts, cls_embs[:, :, :prev_num_classes].repeat(prev_concepts.shape[0], 1, 1)).mean(dim=1)
            ...
            if args.experience > 0:
                prev_concepts_new_embeds = concepts[-prev_concept_values.shape[0]:, :prev_num_concepts, :]
                prev_concept_new_values = concept_values[-prev_concept_values.shape[0]:, :prev_num_concepts]
                prev_class_new_values = torch.bmm(prev_concepts_new_embeds, cls_embs[-prev_concept_values.shape[0]:, :, :prev_num_classes].repeat(prev_concepts.shape[0], 1, 1)).mean(dim=1)
                derpp_loss = F.mse_loss(prev_concept_new_values, prev_concept_values) + F.mse_loss(prev_class_new_values, prev_classes)
                loss += derpp_loss
                log_list.append(('derpp loss', derpp_loss.cpu().detach().numpy()))
    ...
    prev_task_model = deepcopy(mm_model)
    prev_concept_classifier = deepcopy(concept_classifier)
    prev_dataset = train_dataset
    prev_num_concepts = train_dataset.get_concept_count()
    prev_num_classes = len(class_list)
    print(prev_num_concepts, prev_num_classes)
    del exemplars
